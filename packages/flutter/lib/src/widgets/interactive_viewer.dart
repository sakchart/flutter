...
class _InteractiveViewerState extends State<InteractiveViewer> with TickerProviderStateMixin {
  ...
  Matrix4 _matrixTranslate(Matrix4 matrix, Offset translation) {
    ...
    final Matrix4 nextMatrix =
        matrix.clone()..translateD(alignedTranslation.dx, alignedTranslation.dy);
  ...
  Matrix4 _matrixScale(Matrix4 matrix, double scale) {
    ...
    return matrix.clone()..scaleD(clampedScale);
  }
  ...
  Matrix4 _matrixRotate(Matrix4 matrix, double rotation, Offset focalPoint) {
    ...
    return matrix.clone()
      ..translateD(focalPointScene.dx, focalPointScene.dy)
      ..rotateZ(-rotation)
      ..translateD(-focalPointScene.dx, -focalPointScene.dy);
  }
...
Quad _getAxisAlignedBoundingBoxWithRotation(Rect rect, double rotation) {
  final Matrix4 rotationMatrix =
      Matrix4.identity()
        ..translateD(rect.size.width / 2, rect.size.height / 2)
        ..rotateZ(rotation)
        ..translateD(-rect.size.width / 2, -rect.size.height / 2);
